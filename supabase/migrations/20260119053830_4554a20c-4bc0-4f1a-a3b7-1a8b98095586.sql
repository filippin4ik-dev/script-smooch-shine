-- Fix multiplayer_poker_action to use 'value' field instead of 'rank' for cards
CREATE OR REPLACE FUNCTION public.multiplayer_poker_action(p_duel_id uuid, p_user_id uuid, p_action text, p_amount integer DEFAULT 0)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
    ...
    IF v_next_phase = 'showdown' THEN
      ...
      IF NOT COALESCE(v_duel.creator_folded, FALSE) THEN
        ...
        FOR v_i IN 0..jsonb_array_length(v_player_cards) - 1 LOOP
          -- Use 'value' field instead of 'rank'
...
-- Also delete existing games to start fresh
