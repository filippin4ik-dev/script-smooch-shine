-- Update function to exclude freebet bets from tournament results
...
BEGIN
  ...
  IF NEW.status = 'won' AND (OLD.status IS NULL OR OLD.status != 'won') AND (NEW.is_freebet IS NULL OR NEW.is_freebet = FALSE) THEN
...
-- Update function for parlay bets to exclude freebets from tournament results
